...
void MovingObjectFrame::mergeObjects()
{
  ...
  //   return;
  // }
  // std::cout << "objects size: " << objects_localized_.size() <<  std::endl;

  for (std::vector<LocalizationObjectInBox>::iterator it = objects_localized_.begin();
       it != objects_localized_.end(); ++it)
  {
    if (params_->social_filtering_enabled_ && it->object.object_name != "person")
    {
      continue;
    }
...
bool MovingObjectFrame::findMovingObjectByOverlap(
  const ObjectRoi & roi, MovingObject & out)
{
  MovingObjectVector temp_objects = moving_objects_;
  ...
  for (auto t : temp_objects) {
    ...
    if (width <=0 || height <= 0)
      ...
      overlap_ratio = width*height/(width1*height1+width2*height2-width*height);
...
bool MovingObjectFrame::publish(const rclcpp::Publisher<MovingObjectMsg>::SharedPtr & moving_objects_pub_)
{
